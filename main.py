...
#             max_size = 0 # переменная для выбора фото с максимальным размером
#             link = ''
#             likes = item['likes']['count'] # кол-во лайков
#             photo_id = item['id'] # id фото
#             for size in item['sizes']: # выбираем максимальный размер фото
#                 real_size = size['height'] * size['width']  # реальный размер фото
#                 if real_size > max_size: # если реальный размер больше максимального, сохраняем новый максимальный размер и ссылку на фото
#                     max_size = real_size
#                     link = size['url']
#             photos_likes_profile.append([likes, photo_id, link])
#         photos_likes_profile.sort()
...
#         if count_photo == 0: # если вообще нет фото возвратит None
#             return None
#         for i in range(count_photo): # если 1 или 2 фото
#             for item in photo_info['items']:
#                 max_size = 0  # переменная для выбора фото с максимальным размером
#                 link = ''
#                 likes = item['likes']['count']  # кол-во лайков(выше использовалось для сортировки, тут нечего сортировать, т.к. фото меньше 3-х)
#                 photo_id = item['id']  # id фото
#                 for size in item['sizes']: # выбираем максимальный размер фото
#                     real_size = size['height'] * size['width']  # реальный размер фото
#                     if real_size > max_size:  # если реальный размер больше максимального, сохраняем новый максимальный размер и ссылку на фото
